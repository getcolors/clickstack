(ns io.github.getcolors.clickstack.compute
...
(defn requirements [opts]
  (let [ssh (request/source-cidrs opts "ssh-sources" "clickstack-ssh-sources") http (request/source-cidrs opts "http-sources" "clickstack-http-sources")]
    {:single_host true :legacy_state_keys [(str (:profile opts) "/clickstack-infrastructure.tfstate")]
...
(defn attach [opts result]
  (cond
    ...
    (= "destroyed" (:status result)) (assoc opts :green/exit 0 :clickstack/already-destroyed true)
