...
(defn stamp-green [s sha]
  (when (re-find #"\(def \^:private clickstack-sha (?:nil|\"[0-9a-f]{40}\")\)" s)
    (str/replace-first s #"\(def \^:private clickstack-sha (?:nil|\"[0-9a-f]{40}\")\)"
                       (str "(def ^:private clickstack-sha \"" sha "\")"))))

(defn stamp-red [s sha]
  (let [pinned (str "\"package-clickstack-red\": \"github:getcolors/clickstack#" sha "\",")]
    (cond (str/includes? s "\"package-clickstack-red\": null,")
          (str/replace-first s "\"package-clickstack-red\": null," pinned)
          (re-find #"\"package-clickstack-red\": \"github:getcolors/clickstack#[0-9a-f]{40}\"," s)
          (str/replace-first s #"\"package-clickstack-red\": \"github:getcolors/clickstack#[0-9a-f]{40}\"," pinned))))

(def blue-unpinned-meta "# dependencies = []\n# ///")
(defn blue-pinned-meta [sha]
  (str "# dependencies = [\"package-clickstack-blue\", \"blue\", \"colors-compute-blue\"]\n"
       "#\n"
       "# [tool.uv.sources]\n"
       "# colors-compute-blue = { git = \"https://github.com/getcolors/colors-compute.git\", rev = \"3451a05e719b0ad6809f3c88b241a8c010b8f58b\", subdirectory = \"blue\" }\n"
       "# package-clickstack-blue = { git = \"https://github.com/getcolors/clickstack.git\", rev = \"" sha "\", subdirectory = \"blue\" }\n"
...
(defn stamp-blue [s sha]
  ...
  (cond (str/includes? s blue-unpinned-meta)
        (-> s
            ...
            (str/replace-first #"(?s)# UNPINNED:.*?CLICKSTACK_LIB_ROOT=/path/to/clickstack\n"
                               "# Stamped by `bb pin`. CLICKSTACK_LIB_ROOT=/path/to/clickstack still overrides the\n# pin with a working tree.\n"))
        (re-find #"clickstack\.git\", rev = \"[0-9a-f]{40}\"" s)
        (str/replace-first s #"clickstack\.git\", rev = \"[0-9a-f]{40}\""
                           (str "clickstack.git\", rev = \"" sha "\""))))

(def sites
  [{:path "../skills/package-clickstack-green/green" :stamp stamp-green}
   {:path "../skills/package-clickstack-red/red" :stamp stamp-red}
   {:path "../skills/package-clickstack-blue/blue" :stamp stamp-blue}])

(let [dirty (git "status" "--porcelain") sha (git "rev-parse" "HEAD") remotes (git "branch" "-r" "--contains" sha)]
  (cond (seq dirty) (do (binding [*out* *err*] (println "clickstack working tree is dirty; commit before pinning")) (System/exit 2))
        (not (str/includes? (str remotes) "origin/")) (do (binding [*out* *err*] (println "clickstack HEAD is not pushed")) (System/exit 2))
