...
resource "digitalocean_ssh_key" "machine" {
  ...
  public_key = trimspace(file("/home/build-placeholder/.ssh/clickstack-digitalocean-fixture.pub"))
}

resource "digitalocean_droplet" "clickstack" {
  ...
  connection {
    ...
    private_key = file("/home/build-placeholder/.ssh/clickstack-digitalocean-fixture")
  }
...
resource "digitalocean_firewall" "clickstack" {
  name        = "clickstack-digitalocean-fixture-firewall"
  droplet_ids = [digitalocean_droplet.clickstack.id]
  inbound_rule {
    protocol         = "tcp"
    port_range       = "22"
  ...
  # metrics} to the collector and everything else to HyperDX, so the collector's
  # own 4317/4318 never need a rule.
...
output "params" {
  value = {
    provider = "digitalocean"
    ip       = digitalocean_droplet.clickstack.ipv4_address
    user     = "root"
    sudoer   = "root"
    name     = "clickstack-digitalocean-fixture"
