...
resource "vultr_ssh_key" "machine" {
  ...
  ssh_key = trimspace(file("/home/build-placeholder/.ssh/clickstack-fixture.pub"))
}

resource "vultr_firewall_group" "clickstack" {
  description = "clickstack-fixture-firewall"
}

resource "vultr_firewall_rule" "ssh" {
  for_each          = toset(local.ssh_sources)
  firewall_group_id = vultr_firewall_group.clickstack.id
  protocol          = "tcp"
  port              = "22"
...
resource "vultr_instance" "clickstack" {
  ...
  connection {
    ...
    user = "root"
    host = self.main_ip
    private_key = file("/home/build-placeholder/.ssh/clickstack-fixture")
  }
