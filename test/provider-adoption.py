#!/usr/bin/env python3
"""Prove an unchanged existing singleton consumer adopts an isolated library provider.
...
ROOT = APP.parent / 'colors-compute'
...
def main():
    ...
    production = [ROOT/'red/resources', ROOT/'blue/src/colors_compute', ROOT/'green/src/resources/colors_compute']
    before = digest(sources + production)
    ...
    opts = {**json.loads(fixture.stdout),'profile':'adoption', 'provider-compute':PROVIDER, 'provider-backend':'s3', 's3-bucket':'states', 's3-region':'us-east-1',
            'clickstack-host':'clickstack.example.com', 'clickstack-ssh-sources':['192.0.2.0/24'],
            'clickstack-http-sources':[], 'vultr-region':'ewr', 'vultr-plan':'vc2-2c-4gb', 'vultr-os-id':2284}
    with tempfile.TemporaryDirectory(prefix='colors-provider-adoption-') as directory:
        ...
        (consumer/'run.ts').write_text("""import * as c from './src/compute.ts';import {clickstackWorkflow} from './src/workflow.ts';import {run} from 'red/workflow';import {plan_deployment,validate} from 'colors-compute-red';import {readFileSync} from 'node:fs';
const opts={...JSON.parse(readFileSync(process.argv[2],'utf8')),'red/event':'build',workdir:process.argv[3]};try{const result=await run(clickstackWorkflow,opts);if(result['red/exit']!==0)throw Error('build refused');const plan=plan_deployment(opts,c.topology,c.requirements(opts));console.log('ADOPTION:'+JSON.stringify({nodes:plan.cluster.nodes,documents:plan.documents,validation:validate(opts),build_exit:result['red/exit']}));}catch{console.log('ADOPTION:'+JSON.stringify({status:'unsupported'}));}
""")
        (temp/'run.py').write_text("""import asyncio,json,sys
from package_clickstack_blue import compute as c,workflow
...
async def main():
 try:
  result=await run(workflow.clickstack_workflow,o)
  if result.get('blue/exit')!=0:raise ValueError('build refused')
  plan=plan_deployment(o,c.TOPOLOGY,c.requirements(o))
  print('ADOPTION:'+json.dumps({'nodes':plan['cluster']['nodes'],'documents':plan['documents'],'validation':validate(o),'build_exit':result['blue/exit']}))
 except Exception:print('ADOPTION:'+json.dumps({'status':'unsupported'}))
asyncio.run(main())
""")
        (temp/'run.clj').write_text("""(require '[cheshire.core :as j] '[io.github.getcolors.clickstack.compute :as c] '[io.github.getcolors.clickstack.workflow :as w] '[green.workflow :as wf] '[io.github.getcolors.compute :as compute] '[io.github.getcolors.compute-planning :as planning])
(let [opts (assoc (j/parse-string (slurp (first *command-line-args*)) true) :green/event :build :workdir (second *command-line-args*))]
 (println (str "ADOPTION:" (j/generate-string (try (let [result (wf/run w/workflow opts) _ (when-not (= 0 (:green/exit result)) (throw (ex-info "build refused" {}))) plan (planning/plan-deployment opts c/topology (c/requirements opts))] {:nodes (get-in plan [:cluster :nodes]) :documents (:documents plan) :validation (compute/validate opts) :build_exit (:green/exit result)}) (catch Exception _ {:status "unsupported"}))))))
""")
        ...
        for color,result in outputs.items():
            ...
            assert len(result.get('documents',{}).get('nodes',{}))==1 and result['documents']['shared'], (color,result)
            assert list((temp/('build-'+color)).rglob('inventory.json')) and list((temp/('build-'+color)).rglob('compose.yml')), color
            subprocess.run(['python3', str(APP/'scripts/check-compute-plan.py'), str(temp/('build-'+color)/'adoption')],check=True)
            assert len(result.get('nodes',[]))==1, (color,result)
            assert all(n['provider']==PROVIDER for n in result['nodes']), (color,result)
            assert result['nodes'][0]['name']=='adoption', (color,result)
            assert result['nodes'][0]['vpc_ip'] is None
            print(f'{color}: unchanged ClickStack workflow built compute, DNS and Ansible using dependency-only fixture provider')
        assert outputs['red']==outputs['blue']==outputs['green'], 'consumer results disagree across languages'
        assert (consumer/'src/compute.ts').read_bytes()==(APP/'red/src/compute.ts').read_bytes()
    assert digest(sources + production)==before, 'consumer source changed during proof'
