"""The steps and every template spec, the port of io.github.getcolors.clickstack.tools."""
...
infrastructure_tool = "clickstack-infrastructure"
dns_tool = "clickstack-dns"
ansible_tool = "clickstack-ansible"
ansible_local_tool = "clickstack-ansible-local"
ROOT = Path(__file__).parent / "resources"
template_opts = PRESERVE_JINJA_DELIMITERS


def tool_dir(opts: dict, tool: str) -> str:
    return stage_dir(opts, tool, default_profile="clickstack")
...
def zone(opts: dict) -> str | None:
    ...
    return registrable_domain(opts.get("clickstack-host"))
...
async def ansible_step(opts: dict) -> dict:
    ...
    if opts.get("blue/event") == "delete" and not opts.get("ip"):
        ...
        # inventory would fall back to 192.0.2.10. Remove the rendered tree
        # the way a completed cleanup would and let the teardown continue.
        return {**scaffold(opts, ansible_specs(opts)), "blue/exit": 0,
...
endpoint_wired = {"200", "400", "401", "403", "415", "422"}


async def acceptance_step(opts: dict) -> dict:
    """Public health checks after a real create. The end-to-end ingest proof
    runs on the server, inside the playbook, where the generated ingestion key
    lives; what is checked from here is what a user can actually reach: the UI
    over HTTPS and the OTLP receiver behind it."""
    if opts.get("blue/event") != "create":
        return {**opts, "blue/exit": 0}
    base = f"https://{opts.get('clickstack-host')}"
    if not await wait_for(["curl", "-fsS", "-o", "/dev/null", f"{base}/"], 60):
        return {**opts, "blue/exit": 1,
                "blue/err": "HyperDX UI did not become reachable over HTTPS"}
    page = await run(["curl", "-fsS", f"{base}/"])
    otlp = await http_status(
        ["curl", "-s", "-o", "/dev/null", "-w", "%{http_code}",
         "-X", "POST", "-H", "content-type: application/json",
         "--data", '{"resourceLogs":[]}', f"{base}/v1/logs"])
    if not re.search(r"(?i)hyperdx", str(page.out or "")):
        return {**opts, "blue/exit": 1, "blue/err": "the HyperDX UI did not render"}
    if otlp not in endpoint_wired:
        return {**opts, "blue/exit": 1,
                "blue/err": f"the public OTLP endpoint is not wired: /v1/logs returned {otlp}"}
    return {**opts, "blue/exit": 0,
