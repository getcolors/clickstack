"""The graph, the port of io.github.getcolors.clickstack.workflow."""
...
def wire_fn(step: str, run_opts: dict):
    if run_opts.get("blue/event") == "delete":
        return {
            "clickstack/start": (start_step, "clickstack/ansible"),
            "clickstack/ansible": (tools.ansible_step, "clickstack/dns"),
            ...
            "clickstack/dns": (tools.dns_step, "clickstack/ssh-config"),
            "clickstack/ssh-config": (tools.ansible_local_step, "clickstack/infrastructure"),
            "clickstack/infrastructure": (tools.infrastructure_step, "clickstack/ssh-cleanup"),
            "clickstack/ssh-cleanup": (ssh.cleanup_step,),
        }.get(step)
    return {
        "clickstack/start": (start_step, "clickstack/infrastructure"),
        # After compute, which is where the address first exists, and before
        # the stage that converges the machine.
        "clickstack/infrastructure": (tools.infrastructure_step, "clickstack/ssh-config"),
        "clickstack/ssh-config": (tools.ansible_local_step, "clickstack/dns"),
        "clickstack/dns": (tools.dns_step, "clickstack/ansible"),
        "clickstack/ansible": (tools.ansible_step, "clickstack/acceptance"),
        "clickstack/acceptance": (tools.acceptance_step,),
    }.get(step)
...
side_effecting = ["clickstack/infrastructure", "clickstack/dns", "clickstack/ssh-config",
                  "clickstack/ansible", "clickstack/acceptance", "clickstack/ssh-cleanup"]


def create_workflow():
    wf = workflow(start="clickstack/start", wire_fn=wire_fn)
    wf = advice_add(wf, "clickstack/infrastructure", "before", "clickstack.workflow/backend",
                    backend_advice(tools.infrastructure_tool))
    wf = advice_add(wf, "clickstack/dns", "before", "clickstack.workflow/backend",
                    backend_advice(tools.dns_tool))
    return dry_run.advise(progress.advise(wf), side_effecting)


clickstack_workflow = create_workflow()
