...
def wire_fn(step: str, run_opts: dict):
    if run_opts.get("blue/event") == "delete":
        return {
            ...
            "clickstack/infrastructure": (tools.infrastructure_step,),
        }.get(step)
    return {
...
def create_workflow():
    wf = workflow(start="clickstack/start", wire_fn=wire_fn, next_fn=lambda _step, successors, opts: [] if opts.get("clickstack/already-destroyed") or failed(opts) else [(step, opts) for step in (successors or [])])
