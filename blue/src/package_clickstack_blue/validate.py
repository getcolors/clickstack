...
def provider_errors(opts: dict) -> list[str]:
    ...
    name = _s(opts.get(name_key))
    if rule and not placeholder(opts.get(name_key)) and (len(name) > 63 or not rule["re"].fullmatch(name)):
        errors.append(f":{name_key} {rule['message']}")
