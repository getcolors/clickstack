...
def requirements(opts):
    ssh = source_cidrs(opts, 'ssh-sources', 'clickstack-ssh-sources')
    http = source_cidrs(opts, 'http-sources', 'clickstack-http-sources')
    return {'single_host': True, 'legacy_state_keys': [opts['profile'] + '/clickstack-infrastructure.tfstate'],
...
def attach(opts, result):
    ...
    if result['status'] == 'destroyed':
        return {**opts, 'blue/exit': 0, 'clickstack/already-destroyed': True}
    params = node(result)
