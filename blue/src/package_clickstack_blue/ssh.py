...
token), and a cleanup that runs only after a successful destroy. Reusing it rather than
...
``$HOME`` and does not commit rendered output; clickstack does commit goldens,
...
def with_machine_key(opts: dict) -> dict:
    ...
    if not build:
        return opts
    profile = opts.get("profile") or "clickstack"
