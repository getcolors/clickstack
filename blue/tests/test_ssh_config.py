...
from package_clickstack_blue import ssh_config, tools, workflow

# §2 the alias and the identity file


def test_alias_is_the_profile():
    assert ssh_config.host_alias(fixture()) == "clickstack-fixture"
...
def test_identity_file_keeps_the_tilde(tmp_path, monkeypatch):
    ...
    assert ssh_config.identity_file(fixture()) == "~/.ssh/clickstack-fixture"
...
def test_the_marker_is_the_alias_alone():
    ...
    assert ssh_config.begin_marker("clickstack-vultr") == "# BEGIN clickstack-vultr ANSIBLE MANAGED BLOCK"
    assert ssh_config.end_marker("clickstack-vultr") == "# END clickstack-vultr ANSIBLE MANAGED BLOCK"
...
def test_a_foreign_stanza_is_found():
    lines = ["Host other", "    HostName 192.0.2.1", "", "Host clickstack-fixture"]
    assert ssh_config.foreign_stanza_line(lines, "clickstack-fixture") == 4


def test_our_own_block_is_not_foreign():
    alias = "clickstack-fixture"
...
def test_a_block_under_a_retired_marker_is_foreign():
    ...
    alias = "clickstack-vultr"
    lines = [f"# BEGIN clickstack {alias} ANSIBLE MANAGED BLOCK",
             f"Host {alias}",
             f"# END clickstack {alias} ANSIBLE MANAGED BLOCK"]
    assert ssh_config.foreign_stanza_line(lines, alias) == 2


def test_a_multi_pattern_host_line_counts():
    assert ssh_config.foreign_stanza_line(["Host web clickstack-fixture db"], "clickstack-fixture") == 1


def test_an_unrelated_file_is_left_alone():
    assert ssh_config.foreign_stanza_line(["Host build", "Host clickstack-other"],
                                          "clickstack-fixture") is None
...
def test_the_local_play_renders_no_address():
    ...
    assert data["ssh-config-identity-file"] == "~/.ssh/clickstack-fixture"
...
def test_the_local_stage_renders_three_files():
    ...
    assert all("clickstack-ansible-local" in t for t in targets)
...
def test_create_writes_the_block_after_compute_and_before_convergence():
    ...
    assert workflow.wire_fn("clickstack/infrastructure", create)[1:] == ("clickstack/ssh-config",)
    assert workflow.wire_fn("clickstack/ssh-config", create)[1:] == ("clickstack/dns",)
