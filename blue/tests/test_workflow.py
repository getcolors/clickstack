...
def test_graph_orders_the_stack():
    ...
    assert workflow.wire_fn("clickstack/start", create)[1:] == ("clickstack/infrastructure",)
    ...
    assert workflow.wire_fn("clickstack/dns", create)[1:] == ("clickstack/ansible",)
    assert workflow.wire_fn("clickstack/ansible", create)[1:] == ("clickstack/acceptance",)
...
def test_delete_removes_the_key_after_the_compute_destroy():
    ...
    assert workflow.wire_fn("clickstack/start", delete)[1:] == ("clickstack/ansible",)
    assert workflow.wire_fn("clickstack/infrastructure", delete)[1:] == ("clickstack/ssh-cleanup",)
    assert workflow.wire_fn("clickstack/ssh-cleanup", delete)[1:] == ()
...
def test_backend_addresses_key_state_by_profile_and_tool():
    ...
    assert dir.endswith(".colors/clickstack-fixture/clickstack-infrastructure")
