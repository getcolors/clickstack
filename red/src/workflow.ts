...
export function providerValidator(opts: Opts, state: StateRead): string[] {
  ...
  return mismatch.length > 0 ? mismatch : validate.secretErrors(opts);
}
...
export function wireFn(step: string, runOpts: Opts): WireDecl | undefined {
  if (runOpts["red/event"] === "delete") {
    const graph: Record<string, WireDecl> = {
      "clickstack/start": [startStep, "clickstack/ansible"],
      "clickstack/ansible": [tools.ansibleStep, "clickstack/dns"],
      ...
      "clickstack/dns": [tools.dnsStep, "clickstack/ssh-config"],
      "clickstack/ssh-config": [tools.ansibleLocalStep, "clickstack/infrastructure"],
      "clickstack/infrastructure": [tools.infrastructureStep, "clickstack/ssh-cleanup"],
      "clickstack/ssh-cleanup": [ssh.cleanupStep],
    };
    return graph[step];
  }
  const graph: Record<string, WireDecl> = {
    "clickstack/start": [startStep, "clickstack/infrastructure"],
    // After compute, which is where the address first exists, and before the
    // stage that converges the machine.
    "clickstack/infrastructure": [tools.infrastructureStep, "clickstack/ssh-config"],
    "clickstack/ssh-config": [tools.ansibleLocalStep, "clickstack/dns"],
    "clickstack/dns": [tools.dnsStep, "clickstack/ansible"],
    "clickstack/ansible": [tools.ansibleStep, "clickstack/acceptance"],
    "clickstack/acceptance": [tools.acceptanceStep],
  };
  return graph[step];
}
...
function create() {
  let wf = workflow({ start: "clickstack/start", wireFn });
  wf = adviceAdd(wf, "clickstack/infrastructure", "before", "clickstack.workflow/backend",
    backendAdvice(tools.infrastructureTool));
  wf = adviceAdd(wf, "clickstack/dns", "before", "clickstack.workflow/backend",
    backendAdvice(tools.dnsTool));
  return dryRun.advise(progress.advise(wf), sideEffecting);
}

export const clickstackWorkflow = create();
