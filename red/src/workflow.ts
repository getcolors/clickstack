...
export function wireFn(step: string, runOpts: Opts): WireDecl | undefined {
  if (runOpts["red/event"] === "delete") {
    const graph: Record<string, WireDecl> = {
      ...
      "clickstack/infrastructure": [tools.infrastructureStep],
    };
    return graph[step];
  }
  const graph: Record<string, WireDecl> = {
...
function create() {
  let wf = workflow({ start: "clickstack/start", wireFn, nextFn:(_step, successors, opts)=>opts["clickstack/already-destroyed"]||failed(opts)?[]:(successors??[]).map(step=>[step,opts]) });
