...
describe("validate", () => {
  ...
  test("compute name falls back to the profile", () => {
    expect(validate.computeName(doFixture())).toBe("clickstack-digitalocean-fixture");
    expect(validate.computeName(doOptout())).toBe("clickstack-digitalocean-optout");
    expect(validate.computeName(fixture({ "vultr-name": null }))).toBe("clickstack-fixture");
    expect(validate.computeName(fixture({ "vultr-name": "" }))).toBe("clickstack-fixture");
    expect(validate.computeName(fixture({ "vultr-name": "REPLACE_ME" }))).toBe("clickstack-fixture");
    ...
    expect(validate.computeName(doFixture({ "vultr-name": "custom-label" })))
      .toBe("clickstack-digitalocean-fixture");
  });
  ...
  test("the name override is validated against the provider's rules", () => {
    ...
    expect(validate.stateErrors(doFixture({ "digitalocean-name": "click.stack-01" }))).toEqual([]);
  });
  ...
  test("reports all errors at once", () => {
    ...
    for (const part of ["host", "image", "admin-email", "provider-dns", "provider-compute"]) {
  ...
  test("secrets and tofu env follow the selected provider", () => {
    const errors = validate.secretErrors(doFixture()).join("\n");
...
describe("tools", () => {
  ...
  test("infrastructure data reads the selected provider's keys", () => {
    ...
    expect(data["compute-name"]).toBe("clickstack-digitalocean-fixture");
    expect(tools.infrastructureData(fixture())["compute-name"]).toBe("clickstack-fixture");
  });
  ...
  test("fallback params are shaped per provider", () => {
    expect(tools.fallbackParams(fixture())).toEqual({ provider: "vultr", ip: "192.0.2.10",
      user: "root", sudoer: "root", name: "clickstack-fixture" });
    expect(tools.fallbackParams(doFixture())).toEqual({ provider: "digitalocean", ip: "192.0.2.10",
      user: "root", sudoer: "root", name: "clickstack-digitalocean-fixture" });
  });
  ...
  test("a delete without compute skips the remote cleanup", async () => {
    ...
    try {
      ...
      expect(result["clickstack/cleanup"]).toBe("skipped-no-compute");
    } finally {
