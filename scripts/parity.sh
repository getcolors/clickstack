...
build_variant() {
  ...
  (cd "$root/green" && CLICKSTACK_LIB_ROOT="$root" ./green build -f "$tmp/$variant-green.yml" >/dev/null)
  (cd "$root/red" && CLICKSTACK_LIB_ROOT="$root/red" ./red build -f "$tmp/$variant-red.yml" >/dev/null)
  (cd "$root/blue" && uv run python -m package_clickstack_blue build -f "$tmp/$variant-blue.yml" >/dev/null)
...
diff -r "$root/green/src/resources/io/github/getcolors/clickstack" "$root/red/resources"
diff -r "$root/green/src/resources/io/github/getcolors/clickstack" "$root/blue/src/package_clickstack_blue/resources"

echo "green, red, and blue ClickStack artifacts are byte-identical"
