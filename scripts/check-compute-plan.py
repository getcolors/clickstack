...
assert len(nodes) == 1, 'ClickStack must render exactly one node'
...
assert not (root / 'clickstack-infrastructure').exists(), 'old compute stage must not be applied'
assert (root / 'clickstack-dns/backend.tf.json').is_file(), 'application DNS remains separate'
